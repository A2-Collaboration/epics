...
void makeEnh(const Char_t *P="BEAM:CBREM:", const Char_t *G="BEAM:GONI:", Int_t simulated = 0, Int_t period=2){
  Int_t chan;
  ...
  while(1){
    ...
    if(HodoState) epicsGet(HodoScalerPV,kEpicsDOUBLE,nHodo,HodoScaler);
    if(MicroState) epicsGet(MicroScalerPV,kEpicsDOUBLE,nMicro,MicroScaler);
    ...
    //Now read the amo current reference
    ...
    for(int n=0;n<100;n++){                      //loop over list of bad chans, and for those above 5, copy the  
      //cout <<  (int)DeadScalerChans[n] <<  endl;
      if(DeadScalerChans[n] <= 5.0 ) break;   //content of the previous channel
      else hEnh->SetBinContent((int)DeadScalerChans[n],hEnh->GetBinContent((int)DeadScalerChans[n]-1));
    }    
    epicsPut(EnhScalerPV,kEpicsDOUBLE,chan,Enh);

    if(radiatorID&&radiatorIndex){
      ...
      for(int n=hEnh->FindBin(0.8*req_edge);n<hEnh->FindBin(1.2*req_edge);n++){
	if(hEnh->GetBinContent(n)>fitmax){
      ...
      edgeFit->SetRange(fitmin,1.2*fitmin);
      edgeFit->SetParameter(0,fitmax);
      edgeFit->SetParameter(1,fitmin);
      edgeFit->SetParameter(2,4.0);
	...
	maxgrad=0.0;
	for(Double_t edge=fitmin; edge<1.2*fitmin; edge+=0.002*fitmin){
	  if(fabs(edgeFit->Derivative(edge))>maxgrad){
	    maxgrad=fabs(edgeFit->Derivative(edge));
	    coh_edge=edge;
	  }
	}
      }
      ...
      cout << "Peak = " << fitmin << "Edge = " << coh_edge << ",.... wait " << period << "s" << endl;
    }
    hEnh->Draw();
    CbremCanvas->Update();
    gSystem->ProcessEvents();
    gSystem->Sleep(1000*period);
    
    if(heartbeat>0.0) heartbeat=0.0;  //make the heart beat.
    else heartbeat=1.0;
    epicsPut(HeartbeatPV,kEpicsDOUBLE,1,&heartbeat);
  }
}



void simulateTaggerSimple(Double_t counts=100000.0, Double_t sigma = 6.0 ){
  ...
  if(HodoState){
    for(int n=1; n<=hHodoS->GetNbinsX(); n++){
      ...
      if((radiatorID)&&(hodoPeakBin>0)&&(hodoPeakBin<nHodo)){          //if diamond and peak in range
	if(n>=hodoPeakBin) val+= (1000.0/(edge*edge))*lcounts*hHodoS->GetBinWidth(n)*TMath::Gaus(hHodoS->GetBinCenter(n),edge,sigma);
	else val+= (1000.0/(edge*edge))*lcounts*hHodoS->GetBinWidth(n)*TMath::Gaus(hHodoS->GetBinCenter(n),edge,5.0*sigma);
      }
      hHodoS->SetBinContent(n, random->Gaus(val,TMath::Sqrt(val))); //error from sqrt of lcounts
    }
  ...
  if(MicroState){
    for(int n=1; n<=hMicroS->GetNbinsX(); n++){
      ...
      if((radiatorID)&&(microPeakBin>0)&&(microPeakBin<nMicro)){          //if diamond and peak in range
	if(n>=microPeakBin) val+= (1000.0/(edge*edge))*lcounts*hMicroS->GetBinWidth(n)*TMath::Gaus(hMicroS->GetBinCenter(n),edge,sigma);
	else val+= (1000.0/(edge*edge))*lcounts*hMicroS->GetBinWidth(n)*TMath::Gaus(hMicroS->GetBinCenter(n),edge,5.0*sigma);
      }
      hMicroS->SetBinContent(n, random->Gaus(val,TMath::Sqrt(val))); //error from sqrt of lcounts
    }
