...
//int epicsSetMonitor(char *pvname, char ezcatype){  return ezcaSetMonitor(pvname, ezcatype);}
...
float epicsGetTimeout(){return ezcaGetTimeout();}
int   epicsSetTimeout(float sec){return ezcaSetTimeout(sec);}

int   epicsGetRetryCount(){return ezcaGetRetryCount();}
int   epicsSetRetryCount(int retry){return ezcaSetRetryCount(retry);}
